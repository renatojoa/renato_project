class ProgressBarPage {
  ...
  stopBeforeReaching(maxPercentage) {
    const checkAndStop = () => {
      return this.getProgressValue().then(value => {
        if (value >= maxPercentage - 3 && value < maxPercentage) {
          this.stop();
          return true;
        }
    ...
    cy.waitUntil(checkAndStop, {
      timeout: 30000,
      interval: 100,
      errorMsg: `Could not stop before ${maxPercentage}%`
    });
  }
