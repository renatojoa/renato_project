class PracticeFormPage {
  ...
  verifySubmittedData(data) {
    ...
    cy.log(`🖼️ Validando Picture`);
    cy.log(`   Esperado: sample-upload.txt`);
    this.elements.modalTable()
      .contains('td', 'Picture')
      .next()
      .invoke('text')
      .then((actualPicture) => {
        cy.log(`   Recebido: ${actualPicture}`);
        expect(actualPicture).to.equal('sample-upload.txt');
        cy.log(`   ✅ Picture validado com sucesso!`);
      });
  ...
  closeModal() {
    cy.log('🔘 Clicando no botão Close do modal...');
    this.elements.closeModalButton().should('be.visible').click();
    cy.log('✅ Botão Close clicado com sucesso!');
    // Verify modal is no longer visible
    this.elements.confirmationModal().should('not.exist');
    cy.log('✅ Modal fechado e removido da página!');
  }
}
